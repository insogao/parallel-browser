...
- 意图 token 与四类来源（2026-09-16 审计跟进）：每次控制操作生成唯一 `token`，transition 带 `origin`（`explicit`/`auto`/`internal`/`unknown`）+ `source`/`route`/`requestId`/`gen`；`window-minimize`/`window-restore`/`window-corner`/`native-hide|unhide|activate` 先记 `requested` 再记实际 `after`/失败，找不到证据时 origin=unknown 且不编造 source。不变量：只有 `explicit` 意图可 `maximize`/foreground（auto/unknown 降级为 restore/unhide 并记 `policy-downgrade`）；`/api/login`、`/api/console` 要求显式 source，`/api/login` 在启动/显示/激活之前记录意图，托盘自动请求在显式意图 in-flight 时返回 `ignored:explicit-in-flight`。CLI/dashboard/验收探针均带 source。`window-intent-unit` 新增 7 项确定性测试，两个 opt-in 真机探针新增 source/token 直证断言。
...
单元测试为 `tests/*-unit.ts`（capture、windows、extension-dev、cdp、proxy、proxy-race、state-log、window-intent、targets、raf、window-state、launcher、single-instance 等）加 `tests/brand.ts`；当前 78/78 单元 + brand 11 项通过。集成入口 `tests/agent.ts` 的每次 CDP 调用有 15s 上限，回归失败快速报错而非无限挂起；生产代码不设短超时。
...
- 意图归因边界（2026-09-16 审计跟进）：无 source 的 `/api/show` 仍可 restore/unhide，但一律降级为不 maximize/foreground 且日志 origin=unknown（按用户 release gate 必须在验收中判失败）；无 source 的 `/api/login`、`/api/console` 返回 400 零副作用。旧 runtime/旧调用方不带 source 时行为会退化，需 `bl launcher install` + daemon 重启后整体升级。托盘 `poll` 在“受管应用前台且所有窗口已最小化”时的 `NSRunningApplication.hide()` 由 tray 进程直接执行，不进入 daemon state-log（发生在窗口已最小化时，不产生可见状态；记录为残余）。auto 请求现在最多 unhide、不再 activate，第二实例 Dock 点击的接管体验略有变化。pending 意图仅在内存中，daemon 崩溃重启即可清除。
